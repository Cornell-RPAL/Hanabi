...
def convert_annotation(xml_fn):
    ...
    for obj in root.iter('object'):
        ...
        #log(f"{txt_fn} created")
    in_file.close()
    out_file.close()

if len(sys.argv) != 4:
    log(f"Usage: {sys.argv[0]} images_dir classes.names list.txt")
    log(f"Ex: {sys.argv[0]} data/cards/train data/cards.names data/train.txt")
    log("From xml files in images_dir, convert them in txt files with annotation information and build list.txt file")
    sys.exit(1)
images_dir=sys.argv[1]
classes_fn=sys.argv[2]
list_fn=sys.argv[3]
if not os.path.isfile(classes_fn):
    log(f"Classes file {classes_fn} is not a file")
    sys.exit(1)
if not os.path.isdir(images_dir):
    log(f"{images_dir} is not a directory")
    sys.exit(1)
with open(classes_fn,"r") as f:
    classes=f.read().split("\n")
classes=[c for c in classes if c!='']
log(classes,len(classes))
